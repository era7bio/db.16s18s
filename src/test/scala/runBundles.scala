...
case object db16s18s {
  ...
  def pick16SCandidates(user: AWSUser): Seq[String] =
    launch(era7bio.db.both16s18s.test.compats.pick16SCandidates, r3.`2xlarge`)(user)

  def dropRedundantAssignmentsAndGenerate(user: AWSUser): Seq[String] =
    launch(era7bio.db.both16s18s.test.compats.dropRedundantAssignmentsAndGenerate, r3.large)(user)

  def clusterSequences(user: AWSUser): Seq[String] =
    launch(era7bio.db.both16s18s.test.compats.clusterSequences, r3.large)(user)

  def dropInconsistentAssignmentsAndGenerate(user: AWSUser): Seq[String] =
    launch(era7bio.db.both16s18s.test.compats.dropInconsistentAssignmentsAndGenerate, r3.large)(user)
}
